...
\item Vũ Văn Nghĩa: Lập kế hoạch, phân chia công việc, phân chia công việc, xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx, Tìm hiểu nội dung, Làm slide \LaTeX, Lập trình phần ví dụ RSA, \dots
\end{itemize}
\end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{block}{Định thức}
    ...
    % \section{Định thức}
    % \defi{\textbf{Định nghĩa 1.3.} \textit{Với $\{x_1,x_2,...,x_n\} $ là cơ sở của lưới $L$ ta định nghĩa $$det L = |det(x_1,x_2,...,x_n)|\textbf{.}$$}}
    % Về hình học , nó là thể tích của các hình bình hành kéo dài bởi các vector cơ sở $x_1,x_2,...,x_n$.
...
% \textbf{Chú ý:} Các vector không được chuẩn hóa \index{chuẩn hóa} độ dài. Ngoài ra, nếu $x_1,x_2,...,x_n$ là một cơ sở \index{cơ sở} của lưới $L$ thì sau khi trực giao hóa ta thu được các vector $x_1^*,x_2^*,...,x_n^*$ có thể không nằm trong lưới $L$.\\
