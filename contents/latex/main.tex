...
% \chapter{Thuật toán LLL}
% Trong mục này , bài báo cáo sẽ trình bày về thuật toán LLL \index{thuật toán LLL} cho việc rút gọn cơ sở lưới. Thuật toán này được lấy tên theo chữ cái đầu tiên của ba tác giả A.Ken.Lenstra, H.W.Lenstra Jr và L. Lovasz, những người đã giới thiệu thuật toán này đầu tiên năm 1982\cite{8}.\\

% Ý tưởng việc rút gọn cơ sở \index{cơ sở} là thay đổi cơ sở $B$ của lưới $L$ thành cơ sở mới $B'$ ngắn hơn sao cho $|det L |$ không đổi. Rút gọn cơ sở có thể được sử dụng để giải bài toán vector ngắn nhất theo nghĩa là vector cơ sở ngắn nhất ( $b_1$ trong thuật toán giảm lưới gần như là vector ngắn nhất \index{vector ngắn nhất}).


% \begin{figure}[h!]
% \centering
%   \includegraphics[width = 0.9\textwidth]{a.2.png}
...
% $$x_2^* = x_2 - \frac{x_1.x_2}{x_1.x_1}.x_1 \text{.}$$ Đặt $t$ là số nguyên gần nhất với $\dfrac{x_1.x_2}{x_1.x_1}$ . Sau đó, ta thay thế cơ sở $\{x_1,x_2 \}$ bằng cơ sở $$\{x_1, x_2 - tx_1\}\text{.}$$ Sau đó, ta lặp lại quy trình này đến khi không thể giảm cơ sở nữa.
% \vspace{0.5cm}
% \defi{\textbf{Định nghĩa 2.4.} \textit{Một cơ sở $\{x_1,x_2\}$ được gọi là cơ sở giảm khi và chỉ khi \begin{enumerate}[i)]
%     \item $\|x_1\| \leq \|x_2\|$,
...
% \begin{figure}[h!]
% \centering
%   \includegraphics[scale = 1]{a.3.3.png}
%   \caption{Hình ảnh cơ sở giảm và chưa giảm}
%   \label{fig:boat1}
% \end{figure}

% \textbf{Ví dụ} Cho $\{x_1,x_2\}$ là cơ sở của lưới $L$ với $x_1 = (31,59), x_2 = (37,70)$. Thực hiện giảm cơ sở lưới trên.\\
% Ta thấy $\|x_1\| \leq \|x_2\|$. Ta có $\dfrac{x_1.x_2}{x_1.x_1} = \dfrac{5277}{4442} \approx 1,18$\\
...
%     x_2^{''} = x_2^{'} - t_2x_1^{'} = (1,4)
% \end{cases}$$
% \\ + Hoán vị vị trí $x_1^{''}$ và $x_2^{''}$ ta thu được cơ sở $$\begin{cases}
%     x_1^{''}= (1,4)\\
%     x_2^{''}= (6,11)
% \end{cases}$$
...
%     x_2^{(3)} = x_2^{''} - 3.x_1^{''} = (3,-1)
% \end{cases}$$\\
% + Hoán vị 2 vector ta có $$\begin{cases}
%     x_1^{(3)} = (3,-1)\\
%     x_2^{(3)}  = (1,4)
% \end{cases}$$
% \hspace*{1cm}Ta thấy $\dfrac{x_1^{(3)}.x_2^{(3)}}{x_1^{(3)}.x_1^{(3)}} = -\dfrac{1}{10}$
% . \qquad Suy ra $\{x_1^{(3)},x_2^{(3)}\}$ là cơ sở giảm.\vspace{0.5cm}
...
% \defi{\textbf{Thuật toán:} \textit{Giảm lưới 2 chiều.}}
% \begin{itemize}
%     \item Bước 1 : Nhập vào cơ sở lưới 2 chiều ${b_1,b_2} \in R^2$.
%     \item Bước 2 : Nếu $\|b_1\| > \|b_2\|$, ta hoán đổi vị trí $b_1$ và $b_2$.
%     \item Bước 3 : Đặt $t$ bằng phần nguyên $\left|\dfrac{b_1.b_2}{b_1.b_1}\right|$. Nếu $t = 0$, dừng lại và kết luận là cơ sở giảm. Ngược lại, ta tính lại $b_2 = b_2 - t.b_1$ và quay lại bước 2 
% \end{itemize}
% \begin{figure}[h!]
% \centering
%   \includegraphics[scale = 1]{a.10.png}
%   \caption{Thuật toán giảm lưới 2 chiều.}
%   \label{fig:boat1}
% \end{figure}
...
% \newpage
% \section{Thuật toán LLL}
% Việc giảm lưới ở kích thước lớn hơn 2 chiều khó khăn hơn nhiều. Trong nhiều bài toán, cần có một vector ngắn và không nhất thiết vector đó ngắn nhất. Thuật toán LLL \index{thuật toán LLL} sử dụng cách tiếp cận này và tìm kiếm các vector gần như ngắn nhất(trong thời gian đa thức). Phương pháp này được dùng cho nhiều ứng dụng khác nhau.
% \defi{\textbf{Định nghĩa 2.6.} \textit{Tham số rút gọn \index{tham số rút gọn}$\alpha$ là một số thực sao cho $\dfrac{1}{4} < \alpha < 1$. Giá trị chính tắc của $\alpha$ là $\alpha = \dfrac{3}{4}$\cite{6}.}}

% \defi{\textbf{Định nghĩa 2.7.} \textit{Cơ sở $x_1,x_2,...,x_n$ được gọi là một cơ sở $\alpha$-rút gọn nếu nó thỏa mãn: \begin{enumerate}[i)]
%     \item $|\mu_{ij}| \leq \dfrac{1}{2} $ với $1 \leq j < i \leq n$,
%     \item $\|x_i^*\|^2 \geq (\alpha - \mu_{i,i-1}^2)\|x_{i-1}^*\|^2$ với $2\leq i \leq n$.
% \end{enumerate}}}


%  Ta định nghĩa tham số bổ trợ \index{tham số bổ trợ} $\beta$ như sau: \quad $\beta = \dfrac{4}{4\alpha - 1}$.\\
%  \hspace*{0.7cm}Do  \quad $\frac{1}{4} < \alpha < 1$ \quad $\Rightarrow \quad \beta > \dfrac{4}{3}$. \quad Nếu $\alpha = \dfrac{3}{4}$ thì $\beta = 2$.
